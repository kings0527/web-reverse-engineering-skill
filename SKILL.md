...
### 4.5 黑盒可用 vs 白盒纯算：递进式策略

Web逆向的成熟度分两个层级：

| 层级 | 定义 | 产物 | 时间 | 长期价值 |
|------|------|------|------|----------|
| **黑盒可用** | 补环境让原始代码跑起来，实打验证通过 | 可执行的Node脚本 | 1~4h | 3~6月 |
| **白盒纯算** | 理解算法逻辑，纯JS重写，不依赖原始代码 | 可读的算法实现 | 4~12h | 2~3年 |
...
黑盒可用（实打通过）
  ↓
分支诊断：对比浏览器真值 vs Node复刻结果
  ↓
精准定位：找到首个差异的 if/switch 分支
  ↓
局部白盒：改写该分支为纯算法逻辑
  ↓
逐步扩展：重复上述过程
  ↓
完整白盒：所有依赖解析完毕，纯JS可运行
```

#### 分支诊断方法

当黑盒复刻结果与浏览器不一致时：
1. 在浏览器中记录完整执行路径（所有分支决策）
2. 在Node中记录对应路径
3. 对比两条路径的首个分歧点
4. 定位导致差异的变量/条件
5. 修正后重新验证

工具：Chrome DevTools LogPoint记录分支 + diff对比
...
| 类型 | 内容 | 用途 | 更新频率 |
|------|------|------|----------|
| 站点指纹 | 反爬类型、保护厂商、特征关键词 | 快速判型 | 3~6月 |
| 算法库 | 已破解的签名/加密函数 | 避免重复分析 | 持续 |
| 踩坑清单 | 反调试陷阱、环境修补遗漏 | 新手指导 | 每周 |
| 工具评估 | 各工具有效性/性能/可靠性 | 工具选型 | 每月 |
...
site:
  domain: example.com
  protect_type: [jsvmp, wasm, cdp_check]
  difficulty: L4
...
algorithm:
  name: signature_v2
  type: hmac-sha256
  key_generation: md5(uid + timestamp)
  entry_point: window.getSign(data)

exploit_path:
  blackbox_time: 2h
  whitebox_time: 8h
  key_findings:
    - HMAC key从localStorage读取
    - timestamp精度为秒级（非毫秒）
  tools_used: [hook_api, trace_execution, deobfuscate]
```

### 7.3 记忆驱动的决策加速

1. **快速判型**：新目标与记忆中的相似站点对比，跳过侦察阶段
2. **工具预选**：基于历史tools_used推荐最高效组合
3. **参数热启**：已知的签名格式作为初始假设
4. **反绕圈护栏**：检测到与失败案例相似时主动预警
...
| 密码学识别 | references/crypto-identification.md | 密码学常量快速识别指南 |
| 白盒诊断 | references/whitebox-diagnosis.md | 白盒分支诊断精准定位差异 |
